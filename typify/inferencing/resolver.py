...
class Resolver:
	...
	def process_name_binding(self, resolved_target: PackGroup, resolved_value: ReferenceSet):
		...
		def _top_slots(groups: list) -> tuple[int | None, int, int]:
			star_idx = None
		...
		for ref in resolved_value:
			...
			def _bind_one_slot(slot_group, element_refs: ReferenceSet | None):
			...
			if ref.instanceof(TupleType) and ref.store:
				...
				def _element_at(idx: int) -> ReferenceSet | None:
					if 0 <= idx < n:
