...
class SymbolSlotCollector(ast.NodeVisitor):
	...
	def visit_FunctionDef(self, node):
		...
		enclosing = self.current_table.get_latest_definition()
		fd = self.push(ScopeManager.function_table(node, enclosing))
		for k, v in param_map.items():
			vt = fd.add_variable(VariableTable(k))
			vt.add_definition(DefinitionTable(vt.get_enclosing_module(), v.position))
		self.generic_visit(node)
		self.pop()
